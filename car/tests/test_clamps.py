...
def _fuel_scalar(val: float, tid: str = "Injector Flow Scaling") -> Table:
    return Table(tid, "scalar", np.array(float(val)), units="cc/min")


def _afr_map(val: float, load: float = 2.0, rpm: float = 3000.0) -> Table:
    return Table("Primary Open Loop Fueling A_", "map_2d", np.array([[float(val)]]), units="AFR",
...
def _timing_map(val: float, rpm: float = 4500.0, load: float = 2.0) -> Table:
    return Table("Timing Compensation A", "map_2d", np.array([[float(val)]]), units="deg",
                 x_axis=TableAxis("load", (load,), "g/rev"), y_axis=TableAxis("rpm", (rpm,), "rpm"))
...
def test_ve_rate_limit_caps_at_3pct():
    ...
    res = apply_clamps(_fuel_prop([CellEdit("Injector Flow Scaling", 0, 0, 900.0)]), _ctx(ts))  # +20%
    assert res.ok
...
def test_ve_rate_limit_passes_small_change():
    ts = _ts(_fuel_scalar(750.0))
    res = apply_clamps(_fuel_prop([CellEdit("Injector Flow Scaling", 0, 0, 760.0)]), _ctx(ts))  # +1.3%
...
def test_ve_rate_limit_negative_direction():
    ...
    res = apply_clamps(_fuel_prop([CellEdit("Injector Flow Scaling", 0, 0, 500.0)]), _ctx(ts))  # -50%
    assert res.clamped_edits[0].new_value == 970.0  # 1000*0.97, sign preserved


def test_knock_aborts_everything():
    ts = _ts(_fuel_scalar(750.0))
    res = apply_clamps(_fuel_prop([CellEdit("Injector Flow Scaling", 0, 0, 760.0)]),
...
def test_afr_floor_floors_lean_at_boost():
    ...
    res = apply_clamps(_fuel_prop([CellEdit("Primary Open Loop Fueling A_", 0, 0, 13.5)]), _ctx(ts))
...
def test_afr_floor_ignores_below_boost_threshold():
    ...
    res = apply_clamps(_fuel_prop([CellEdit("Primary Open Loop Fueling A_", 0, 0, 15.0)]), _ctx(ts))
...
def test_timing_ceiling_floors():
    ...
    res = apply_clamps(_timing_prop([CellEdit("Timing Compensation A", 0, 0, 20.0)]),
...
def test_fuel_before_timing_defers():
    ...
    res = apply_clamps(_timing_prop([CellEdit("Timing Compensation A", 0, 0, 12.0)]),
...
def test_steady_before_transient_defers_then_passes():
    ts = _ts(_fuel_scalar(750.0))
    prop = Proposal("p", "accel", (CellEdit("Injector Flow Scaling", 0, 0, 755.0),),
...
def test_boost_gate_defers_until_all_preconditions():
    ts = _ts(_fuel_scalar(15.0, tid="Wastegate Duty"))
    prop = _boost_prop([CellEdit("Wastegate Duty", 0, 0, 16.0)])
