...
def _fuel_scalar(val):
    return Table("Injector Flow Scaling", "scalar", np.array(float(val)), units="cc/min")
...
def test_ve_rate_bound_always(cur, new):
    ...
    ts = TableSet({"Injector Flow Scaling": _fuel_scalar(cur)})
...
def test_knock_empties_always(new):
    ...
    ts = TableSet({"Injector Flow Scaling": _fuel_scalar(750.0)})
...
def test_afr_floor_never_lean_at_boost(target, cur):
    ...
    t = Table("Primary Open Loop Fueling A_", "map_2d", np.array([[cur]]), units="AFR",
              x_axis=TableAxis("load", (2.0,), "g/rev"), y_axis=TableAxis("rpm", (3000.0,), "rpm"))
    ts = TableSet({t.table_id: t})
    prop = Proposal("p", "s", (CellEdit("Primary Open Loop Fueling A_", 0, 0, target),),
                    "fuel", "algorithm:test")
