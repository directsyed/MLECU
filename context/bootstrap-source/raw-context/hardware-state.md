...
**Authoritative current state as of June 19, 2026.** This file supersedes all hardware details in older project docs (master-context.md, project-master-plan-v2.md, and all prior handoffs) where they conflict. This is the most frequently-changing context — keep it updated as the build evolves.
...
## 5. Immediate driver/boot state (IN PROGRESS at session end)

- **Fans ran at 100% (jet-engine)** — expected T630 behavior with an unrecognized PCIe card. **Fix: `ipmitool raw` manual fan control.** Install ipmitool; `0x30 0x30 0x01 0x00` → manual mode; `0x30 0x30 0x02 0xff 0x1e` ≈ 30%; revert with `...0x01 0x01`. **CAUTION: manual mode disables auto-ramp — raise fan % before any GPU stress load or watch temps.** May need `sudo modprobe ipmi_devintf ipmi_si` first.
- **`nvidia-smi` not yet installed** (fresh OS, no driver). Plan:
  1. `lspci | grep -i nvidia` — confirm the 3090 is on the bus FIRST (bus-level check).
  2. `sudo apt install nvidia-driver-580-server -y` — the full DRIVER (not just nvidia-utils), `-server` headless variant. (If apt can't find it, `sudo ubuntu-drivers autoinstall`.)
  3. **Reboot** (kernel module needs clean load).
  4. `nvidia-smi` → should show RTX 3090 / 24576 MiB.
  - If "no devices found": check lspci (if lspci sees it but nvidia-smi doesn't = driver/module; if lspci doesn't = hardware seating/power). Secure Boot N/A (legacy boot).
- **NOT YET DONE in T630:** confirm 3090 enumerates; quiet fans; run **gpu-burn + memtest_vulkan IN the T630** to get the **real in-chassis memory-junction temp** (settles the repad question). Linux monitoring: `nvidia-smi --query-gpu=temperature.gpu,temperature.memory,clocks.sm,power.draw,utilization.gpu --format=csv -l 2`.
